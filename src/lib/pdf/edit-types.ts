export type EditOp =
  ...
  | { type: 'text_overlay'; page: number; coverRect: [number, number, number, number]; text: string; fontSize: number; x: number; y: number; bold?: boolean; italic?: boolean; color?: [number, number, number] }
