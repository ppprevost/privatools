...
export default function ConvertToJpg() {
  ...
  const worker = useWorker({
    createWorker: () =>
      new Worker(new URL('../../lib/workers/convert-to-jpg.worker.ts', import.meta.url), { type: 'module' }),
  });
  ...
  return (
    <div className="space-y-6">
      {!file && <DropZone accept=".png,.webp,.heic,.heif,.bmp,.gif" onFiles={handleFiles} compact />}
      ...
      {file && (
        <>
          ...
          {!worker.isProcessing && !resultBlob && (
            <Slider label="JPG Quality" value={quality} min={10} max={100} step={5} unit="%" onChange={setQuality} />
          )}
          ...
          {!worker.isProcessing && !resultBlob && (
            <div className="flex justify-center">
              <Button onClick={handleConvert} size="lg">Convert to JPG</Button>
            </div>
          )}
