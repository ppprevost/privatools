...
const shareTargets = [
  ...
  { name: 'X', icon: XIcon, buildUrl: (url: string, text: string) => `https://x.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}` },
  { name: 'Facebook', icon: FacebookIcon, buildUrl: (url: string) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}` },
...
export default function DownloadButton({ blob, filename }: Readonly<DownloadButtonProps>) {
  ...
  const handleEmail = () => {
    window.location.href = `mailto:?subject=${encodeURIComponent(shareText)}&body=${encodeURIComponent(pageUrl)}`;
    setShowMenu(false);
  };
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="flex items-center gap-3 relative">
        ...
        {showMenu && (
          <div ref={menuRef} className="absolute top-full right-0 mt-3 bg-white rounded-2xl border-[3px] border-slate-900 shadow-[var(--shadow-brutalist-md)] p-2 min-w-[220px] z-50">
            <div className="flex items-center justify-between px-3 py-2 mb-1">
              <span className="text-sm font-black text-slate-900">Share this tool</span>
              <button onClick={() => setShowMenu(false)} className="text-slate-400 hover:text-slate-900 transition-colors cursor-pointer">
                <X size={16} />
              </button>
            </div>
            <div className="grid grid-cols-4 gap-1 px-2 mb-2">
              {shareTargets.map(({ name, icon: Icon, buildUrl }) => (
                <a
                  key={name}
                  href={buildUrl(pageUrl, shareText)}
                  target="_blank"
            ...
            <div className="border-t border-slate-200 pt-1">
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors text-left cursor-pointer"
              >
                {copied ? <Check size={18} className="text-emerald-500" /> : <Copy size={18} className="text-slate-600" />}
                <span className="text-sm font-bold text-slate-700">{copied ? 'Copied!' : 'Copy link'}</span>
              </button>
            </div>
          </div>
        )}
